...
`.trim()
    await conn.sendButtonImg(m.chat, await (await fetch(json.result.image[0])).buffer(), ayaka, 'Kurumiloli Bot Downloader', 'DOWNLOAD', `.nhd ${args[0]}`, { thumbnail: Buffer.alloc(0) })
}
