...
`.trim()
    await conn.sendButtonImg(m.chat, await (await fetch(json.result.image[0])).buffer(), ayaka, 'Kurumiloli Downloader', 'DOWNLOAD', `.nhd ${args[0]}`,m, { thumbnail: Buffer.alloc(0) })
}
