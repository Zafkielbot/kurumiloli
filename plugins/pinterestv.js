...
let handler = async (m, { conn, args, usedPrefix, command }) => {
    ...
    pin(args[0]).then(async res => {
        ...
        await conn.sendVideo(m.chat, json.data.url, `*Zafkiel*`, m, { thumbnail: Buffer.alloc(0) })
    })

}
handler.help = ['pinterestvideo','pinv'].map(v => v + ' <url>')
handler.tags = ['downloader']
handler.command = /^pinterestvideo|pinv$/i
